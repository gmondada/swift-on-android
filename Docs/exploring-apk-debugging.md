...
This seems to come from `lldb-server` which, on connection, reports having a process ready for debugging on that GDB socket (on qQueryGDBServer), which is not true. We use the `lldb-server` provided by the NDK. Moving to a more recent NDK does not change the behavior. Same with `lldb-server` embedded in Android Studio. Same with `lldb-server` compiled from source code.

Workaround:

Just ignore the error.

If the `platform connect...` command is in a command file, run the file with lldb command `command source -e 0 my_file`. If the command is in the `launch.json` (VS Code), prefix it with `!`.
